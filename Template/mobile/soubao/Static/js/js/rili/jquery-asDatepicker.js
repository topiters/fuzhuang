...
(function($, document, window, undefined) {
    ...
    Plugin.prototype = {
        ...
        _generateDaypicker: function(index) {
            this._generateHeader(index, LABEL[this.options.lang].months[this._date.currentMonth[index]] + '月 ' + this._date.currentYear[index]+'年');
            this.daypickers.eq(index).html(this._generateDays(index));
            monthz = this._date.currentMonth[index];
            yearz = this._date.currentYear[index];
        },
        ...
        _generateDays: function(index) {
            ...
            for (var j = 0; j < 42; j++) {
                ...
                var ss = $('#disabled').children().length;
                var y = $('#disabled').children('input').eq(xxi).attr('data-year');
            	var m = $('#disabled').children('input').eq(xxi).attr('data-month')-1;
            	var d = $('#disabled').children('input').eq(xxi).attr('data-day');
            	if(year==y&&month==m&&day==d){
            		html += '<span class="addstyle calendar_otherMonth">' + content + '</span>';
            		xxi++;
            		
            	}else{
            		html += '<span class="addstyle ' + className + '">' + content + '</span>';
               	}
            	if(xxi==ss){
            		xxi=0;
            	}
