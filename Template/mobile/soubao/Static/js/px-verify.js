...
var PXVerify = {
	...
	ShowImageVerify : function(phone,forgot,callback){
		var box = $('<div class="send-phone-verify"><h3>请输入验证码<a></a></h3><p><span>验证码：</span><input type="text"/><img title="点击刷新" src="'+DOMIN.MAIN+'reg_mobile.htmlvercode""></p><div><span></span><a></a></div></div>').appendTo('body').css({"z-index":"5001",top:$(window).scrollTop()+$(window).height()/2-80});
	...
	//		验证成功后回调函数(function)参数：是否成功、相关信息(错误提示信息)、返回原始数据
	//返回：错误信息(string)，正确返回null
	SendPhoneCode:function(phone,exist,callback,url,_data){
		var _this = this;
		phone = _this._trim(phone);
		if(phone==''){
			return '手机号码不能为空！';
		...
		$.ajax({url:DOMIN.MAIN+'/register?jsoncallback=?',data:{act:'get_rand_code'},dataType:"json",success:function(data){
			url = url||DOMIN.MAIN+'/register?jsoncallback=?';
			_data.act = _data.act||'get_code';
			_data.rand = data.code;
			_data.phone = phone;
			_data.forgot = ((exist==true)?1:0);
			$.ajax({
				url:url,
				type:'GET',
				data:_data,
	...
	//		验证成功后回调函数(function)参数：是否成功、相关信息(错误提示信息)、返回原始数据
	//返回：错误信息(string)，正确返回null
	Login:function(user,password,remember,ajax,callback,url){
		var _this = this;
