...
class ReportController extends BaseController{
	...
	public function finance(){
		$sql = "SELECT sum(order_amount) as goods_amount,sum(shipping_price) as shipping_amount,FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap from  __PREFIX__order ";
		...
		$res = M()->cache(true)->query($sql);//订单数,交易额
		
		foreach ($res as $val){
			$arr[$val['gap']] = $val['goods_amount'];
			$brr[$val['gap']] = $this->get_order_cost($val['order_id']);
		...
		for($i=$this->begin;$i<=$this->end;$i=$i+24*3600){
			...
			$date = date('Y-m-d',$i);
			$list[] = array('day'=>$date,'goods_amount'=>$tmp_goods_amount,'amount'=>$tmp_amount,'shipping_amount'=>$tmp_shipping_amount,'end'=>date('Y-m-d',$i+24*60*60));
			$day[] = $date;
		}
	...
	public function get_order_cost($order_id){
		$res = M('order_goods')->where("order_id=$order_id")->select();
		$cost = 0;
		if($res){
			foreach ($res as $val){
				$cost += $val['goods_num']*$val['cost_price'];
			}
		}
		return $cost;
	}
