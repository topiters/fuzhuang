...
class GoodsController extends BaseController {
    ...
    public function addEditCategory(){
            ...
            if(IS_GET)
            {
                ...
                //$goods_category_info[filter_spec] = explode(',', $goods_category_info[filter_spec]);
                //$goods_category_info[brand_id_arr] = explode(',', $goods_category_info[brand_id_str]);
                
                
                //$cat_list = $GoodsLogic->goods_cat_list();
                //$goodsType = M("GoodsType")->select(); // 商品规格                
            ...
            if($_GET['is_ajax'] == 1)
            {
                ...
                //$filter_spec = implode(',', $_POST['spec_id']);// 商品分类的帅选规格                               
                //$filter_attr = implode(',', $_POST['attr_id']);// 商品分类的帅选属性                
                ...
                if(!$GoodsCategory->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                    ...
                    //$GoodsCategory->filter_spec = $filter_spec ? $filter_spec : ''; // 代码上方71行 验证前的结果                    
                    //$GoodsCategory->filter_attr = $filter_attr ? $filter_attr : '';                    
    ...
    public function addEditGoods(){
            ...
            {                
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                    ...
                    //  form表单提交
                   // C('TOKEN_ON',true);
                    $Goods->promote_start_time = strtotime($Goods->promote_start_time);
                    $Goods->promote_end_time = strtotime($Goods->promote_end_time);
                    $Goods->is_promote = empty($Goods->is_promote) ? 0 : 1;
