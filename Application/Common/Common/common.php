 ...
 * 给订单送券送积分 送东西
 */
function order_give($order_id)
{
    // 找出对应的订单
    $order = M('order')->where("order_id = '$order_id'")->find();
    //根据订单赠送符合条件优惠券 // select * from  `tp_coupon` where `type` = 1 and send_start_time > 现在 and  现在  < send_end_time limit 1
    $coupon = M('Coupon')->where("`condition` <= {$order['goods_price']} and `type` = 1 and ".time()." > send_start_time and  ".time()."  < send_end_time")->find();
    if($coupon)
    {
        $data4['cid'] = $coupon['id'];
        $data4['type'] = $coupon['type'];
        $data4['uid'] = $order['user_id'];
        M("CouponList")->add($data4);
    }
    //$points = $order[order_amount] * tpCache('shopping.point_rate'); // 积分换算比例    
    $points = $order[order_amount] * 1; // 积分换算比例
    // 赠送对应的积分
    accountLog($order[user_id], 0,$points,"下单赠送积分");
}
