...
class UsersLogic extends RelationModel
{
    ...
    public function login($username,$password){
        ...
        $user = M('modelusers')->where("mobile='{$username}' OR email='{$username}'")->find();
        if(!$user){
    ...
    public function app_login($username,$password){
        ...
        if(!$user){
           ...
           $result = array('status'=>-3,'msg'=>'账号异常已被锁定！！！');
            ...
            M('modelusers')->where("user_id = {$user['user_id']}")->save(array('token'=>$user['token'],'last_login'=>time()));
            $result = array('status'=>1,'msg'=>'登陆成功','result'=>$user);
        }
        return $result;
    }    
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            if($map['first_leader'])
            {
                $first_leader = M('modelusers')->where("user_id = {$map['first_leader']}")->find();
    ...
    public function reg($username){
        ...
        $user = M('modelusers')->where("user_id = {$user_id}")->find();
     ...
     public function get_info($user_id){
        ...
        $user_info = M('modelusers')->where("user_id = {$user_id}")->find();
        if(!$user_info)
            return false;
...
//        $type = I('get.type',0);
    ...
    public function update_email_mobile($email_mobile,$user_id,$type=1){
        ...
        $is_exist = M('modelusers')->where($condition)->find();
        if($is_exist)
            return false;
        unset($condition[$field]);
        $condition['user_id'] = $user_id;
        $validate = $field.'_validated';
        M('modelusers')->where($condition)->save(array($field=>$email_mobile,$validate=>1));
        return true;
    }
    ...
    public function update_info($user_id,$post=array()){
        $model = M('modelusers')->where("user_id = ".$user_id);
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        $row = M('modelusers')->where("user_id='{$user_id}'")->save(array('password'=>encrypt($new_password)));
        if(!$row)
