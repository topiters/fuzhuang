...
class CartController extends MobileBaseController {
    ...
    public function cart2()
    {
        ...
        $sql = "select c1.name,c1.money,c1.condition, c2.* from __PREFIX__coupon as c1 inner join __PREFIX__coupon_list as c2  on c2.cid = c1.id and c1.type in(1,2,3) where c2.uid = 1 and ".time()." > c1.use_start_time  and ".time()." < c1.use_end_time and c1.condition <= {$result['total_price']['total_fee']}";
    ...
    public function cart4(){
        ...
        $paymentList = M('Plugin')->where("`type`='payment' and status = 1 and  scenario in(0,1)")->select();        
